...
public class Subpopulation implements Group
    ...
    /** Do we remove Phenotypic similar individuals? Inspired by Branke 2015 */
    public Boolean isDuplicateRemoval = false;
    public int numDuplicateRemovalRetries = 1000;
    ...
    public static final String P_DUPREMOVALS = "duplicate-removals";
    public static final String P_DUPREMOVALStries = "duplicate-removals-tries";
    public static final String P_DATATYPE = "eval.problem.data";
    ...
    */
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        // Do we employ duplicate removal strategy
        isDuplicateRemoval = state.parameters.getBoolean(
            base.push(P_DUPREMOVALS),def.push(P_DUPREMOVALS), false); 
        
        //Parameter p =state.parameters.getString(new Parameter("eval.problem.Simulation.ObjectiveType"), null);
        dataType =state.parameters.getString(new Parameter(P_DATATYPE), null);
        
        this.numDuplicateRemovalRetries = state.parameters.getInt(
                base.push(P_DUPREMOVALStries),def.push(P_DUPREMOVALStries),1000);
            if (numDuplicateRemovalRetries < 0) state.output.fatal(
                "The number of retries for DuplicateRemoval must be an integer >= 0.\n",
                base.push(P_RETRIES),def.push(P_RETRIES));
    ...
    public void populate(EvolutionState state, int thread)
        ...
        for(int x=start;x<individuals.length;x++) 
            ...
            for(int tries=0; 
            	...
            	//I want to test a bunch of dummy operations, if output ranking is same as existed individuals, we generate a new one
            	...
            	{    
            		...
            		if(dataType.equals("ec.app.JSP.DoubleData"))
            		{
            			//Cheng Zhang
            			rank = drWrapper.calculateRank(((GPIndividual)individuals[x]).trees[0].child);
            		}            			
            		else if(dataType.equals("ec.app.RS.DoubleData"))
            		{
            			//Lingxuang Liu RSGP
//                		ec.app.RS.DummyOperationWrapper dow=new ec.app.RS.DummyOperationWrapper();
//                		rank = String.valueOf(dow.GetFitnessOfGPNode(((GPIndividual)individuals[x]).trees[0].child).doubleValue());
            		}
            		...
            		while(true)
            		{  
            			isExisted = drWrapper.addRank(rank);
            			
            			//is can be accepted as a new individual
            			if(isExisted == 0)
            			{            				         				
            				break;
            			}
