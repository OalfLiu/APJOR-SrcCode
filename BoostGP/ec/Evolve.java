...
public class Evolve 
    ...
    */
    ...
    public static EvolutionState initialize(ParameterDatabase parameters, int randomSeedOffset, Output output)
        ...
        //llx 2019-01-22 perform evaluation and breed with all threads
        int numThreads = Runtime.getRuntime().availableProcessors();
		String sizeStr = System.getProperty(Evolve.class.getName()+".numThreads");
		if (sizeStr != null)
			numThreads = Integer.parseInt(sizeStr.trim()); 
		breedthreads=numThreads;
		evalthreads=numThreads;	
 ...
 */
    ...
    public static void main(String[] args)
        ...
        for(int job = currentJob ; job < numJobs; job++)
            ...
            // try
                ...
                //2019-03-27 setup machStatsMatrix for Rule assignment problem;
                setupMachStatsMatrix(state);
    ...
    public static void setupMachStatsMatrix(EvolutionState state)
	{
		...
		e.setSimulationLength(60*24*365*6);
		e.setMaxJobsInSystem(1600);
		e.setEnableLookAhead(false);  

		ExtendedJobStatCollector stats = new ExtendedJobStatCollector();
		//stats.setInitialPeriod(60 * 24 * 365);
...
//		mre.printResults();
		...
		for (String k : res.keySet()) {
			if(++counter==res.size()) break;
			if (counter>21 && k != Experiment.RUNTIME) {
				Object v = res.get(k);
		...
		if (valStatNames.size() > 0) {
			state.machStatsMatrix =new SummaryStat[valStatNames.size()/7][7];
			int i=0,j=0;
			for (String k : valStatNames) { 
				state.machStatsMatrix[i][j]=(SummaryStat) res.get(k);
				if(++j>=7)
				{
					i++;
					j=0;
