...
import ec.util.*;
import jasima.core.experiment.AOAPNewRun;
import jasima.core.experiment.KGNewRun;
import jasima.core.experiment.OCBASSNewRun;
...
public class Evaluator_OCBASSNewRun extends SimpleEvaluator {
...
//		this.UtilizationLevel = state.parameters.getDouble(new Parameter("eval.problem.Simulation.UtilizationLevel"), null);
	...
	protected void evalPopChunk(EvolutionState state, int[] numinds, int[] from, int threadnum, SimpleProblemForm p) {
		...
		for (int pop = 0; pop < len; pop++) {
			...
			run.init(this.warmupStageBudget, this.firstStageBudget, this.IterationBudget);
			run.runEvaluation();
