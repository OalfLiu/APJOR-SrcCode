...
public class EvolutionState implements Singleton
    ...
    /** 2017.10.17 where we store all the individuals during Genetic Programming Run  **/
    public ArrayList<GPIndividual> AllGPIndividuals;
    public HashMap<Integer, SummaryStat> AllIndividualsStats;
    
    //2019-03-27 lingxuan liu
    public SummaryStat[][] machStatsMatrix;
    
    public ArrayList<GPIndividual> totalGPIndividuals;  
    //We use it to store fitness of all candidate rules to avoid same rules
    public ArrayList<SummaryStat> allPhenotypicSummarys;
    //We use it to store fitness of new population candidate rules to avoid same rules
    public HashSet<Double> tempPhenotypicFitness;
    ...
    */
    ...
    public void run(int condition)
        ...
        String mode=this.parameters.getString(new Parameter("Mode"),null);
        ...
        {
        	//String xample = this.parameters.getString(new Parameter("Example"),null);
        	String[] xample = (this.parameters.getString(new Parameter("Example"),null)).split(",");
			
    		for(int i=0;i<xample.length;i++)
    		{
    			switch(xample[i])
            	{
