...
public class DuplicateRemovalMutationPipeline extends MutationPipeline {
	...
	public int produce(final int min, final int max, final int start, final int subpopulation, final Individual[] inds,
		...
		for (int q = start; q < n + start; q++) {
			...
			int numOfRetries = 0;
			doWrapper.Init();
			doWrapper.isIndividualSame(state,i.trees[0].child);
			// pick a node
			...
			while (true) {
				...
				if (j != null) {
					isSubDuplicated = doWrapper.isFitnessExisted(doWrapper.GetFitnessOfGPNode(state,j.trees[0].child));
				}

				if (numOfRetries > 30 || !isSubDuplicated) {
					break;
				}
