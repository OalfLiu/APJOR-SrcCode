...
import ec.util.*;
import jasima.core.experiment.OCBANewRun;
...
public class OCBAEvaluator extends SimpleEvaluator {
	...
	public void setup(final EvolutionState state, final Parameter base) {
		...
		this.OCBAIterationBudget = state.parameters.getInt(base.push(P_Simulation_OCBA_IterationBudget), null);
	...
	protected void evalPopChunk(EvolutionState state, int[] numinds, int[] from, int threadnum, SimpleProblemForm p) {
		...
		for (int pop = 0; pop < len; pop++) {
			...
			Boolean UniqueSeed = state.parameters.getBoolean(new Parameter("eval.problem.Simulation.UniqueSeed"), null, true);			
			if(UniqueSeed)
				run.InitilizeSeed = this.InitialSeed;
			else
			...
			run.init(this.warmupStageBudget, this.firstStageBudget, this.OCBAIterationBudget);
			run.runEvaluation();
