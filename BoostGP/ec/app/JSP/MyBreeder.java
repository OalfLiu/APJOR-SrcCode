...
public class MyBreeder extends Breeder
    {
    ...
    public Population breedPopulation(EvolutionState state) 
    	...
    	state.tempPhenotypicFitness = new HashSet<Double>();
    ...
    protected void breedPopChunk(Population newpop, EvolutionState state, int[] numinds, int[] from, int threadnum) 
        {
    	  SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");// �������ڸ�ʽ
       	state.output.println("Cross operation begins: " + df.format(new Date()), 0);
        for(int subpop=0;subpop<newpop.subpops.length;subpop++)
     	...
     	state.output.println("Cross operation ends: " + df.format(new Date()), 0);
        }
        ...
        for(int sub=0;sub<state.population.subpops.length;sub++) 
            ...
            if (numElites(state, sub)==1)
                ...
                for(int x=inds.length-numElites(state, sub);x<inds.length;x++)
                {
    				...
    				doWrapper.Init();
                    state.tempPhenotypicFitness.add(doWrapper.getFitnessInDouble(state,((GPIndividual) (inds[x])).trees[0].child));
                }
            }
