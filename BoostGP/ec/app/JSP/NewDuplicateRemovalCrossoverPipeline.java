...

	{
		...
		do
		{
			for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
			{
				...
				if(parents[0] != null)
				{
					parentFitness.add(doWrapper.getFitnessInDouble(state,parents[0].trees[0].child));
				}
				
				if(parents[1] != null)
				{
					parentFitness.add(doWrapper.getFitnessInDouble(state,parents[1].trees[0].child));
				}
				while(true)
				{
					GPNode p1 = null;
					...
					if(j1 != null)
					{
						curFitness1 = doWrapper.getFitnessInDouble(state,j1.trees[0].child);	
					...
					if(j2 != null)
					{
						curFitness2 = doWrapper.getFitnessInDouble(state,j2.trees[0].child);	
				...
				inds[q] = j1;           			
				state.tempPhenotypicFitness.add(doWrapper.getFitnessInDouble(state,j1.trees[0].child));
				...
				if (q<n+start && !tossSecondParent)
				{
					...
					inds[q] = j2;              		
					state.tempPhenotypicFitness.add(doWrapper.getFitnessInDouble(state,j2.trees[0].child));
					q++;
				}
			}
			return n;
