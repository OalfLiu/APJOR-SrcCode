...
public class TextFileReader {
	...
	private int numMachines,numRoutes;
	...
	protected String readJobs(BufferedReader r, String s) throws IOException {
		if (JOB_SECT_MARKER.equalsIgnoreCase(s))
			...
			s = readDynJobs(r);
		else if ("arrivalRates".equalsIgnoreCase(s))
			s = readArrivalRates(r);
		else
	...
	private String readArrivalRates(BufferedReader r) throws IOException {
		// read job specs, adjust route number to be a valid array index
		String s = Util.nextNonEmptyLine(r);
		if (s == null)
			return s;
		
		DblStream[] arrivalStream=new DblStream[numRoutes];
		double[] arrivalRates = Util.parseDblList(s);
		
		for(int i=0;i<numRoutes;i++)
		{
			arrivalStream[i]=new DblDistribution(new ExponentialDistribution(1440d/arrivalRates[i]));
		}
		
		data.setArrivalRates(arrivalStream);
