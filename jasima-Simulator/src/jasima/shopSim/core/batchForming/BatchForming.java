...
public abstract class BatchForming implements Cloneable, Serializable {
	...
	public double preTestObj;
	public double preTestStd;
	public double preTestFlowTime;
	public double preTestTPTime;
	...
	public PrioRuleTarget nextBatch() {
		...
		//2019-05-03 update job size for each job
		for(int i =0;i<getOwner().queue.size();i++) {
			getOwner().queue.get(i).jobSize=1.0/(double)getOwner().queue.get(i).getCurrentOperation().maxBatchSize;
		}
...
//		op.procTime = opJ.procTime;
		op.procTime = b.procTimeStat.max();
	...
	private boolean checkBatchData(Batch b) {
		...
		//llx 20190110 注释掉，是因为同一个family中的部件，其热处理setup time processtime有可能不同
