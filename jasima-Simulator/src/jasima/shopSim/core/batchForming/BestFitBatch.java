 ...
 *          "$Id$"
 */
public class BestFitBatch extends BatchForming {
	...
	public BestFitBatch(PR batchSeqDR) {
		super();  
	...
	public void formBatches() {
		...
		for (int i = 0; i < numJobs; i++) {
			...
			List<Batch> famBatchList=new ArrayList<Batch>();
			Batch batch = new Batch(j.getShop()); 
			batch.addToBatch(j);
			famBatchList.add(batch);
			...
			for (int n = i + 1; n < numJobs; n++) {
				...
				double minresidual=1.1;
				int bestidx=0;
				
				for(int k=0;k<famBatchList.size();k++) {					
					//add to the best fit batch
					double resi = 1.0-(famBatchList.get(k).fullnessStat.sum()+1.0/(double)j2.getCurrentOperation().maxBatchSize);
					if (!j2.isFuture() && resi>=0.0) {
						if(resi<minresidual) {
							minresidual=resi;
							bestidx=k;
						}
...
//						} 
				...
				if(isFormed) {
					famBatchList.get(bestidx).addToBatch(j2);
				}
				else if(!isFormed) {
					Batch newbatch = new Batch(j.getShop()); 
					newbatch.addToBatch(j2);
					famBatchList.add(newbatch);
				}
...
//				}
			...
			for(int k=0;k<famBatchList.size();k++) {
				if(famBatchList.get(k).numJobsInBatch()>0)
					templist.add(batch);
			}
		}
		
		// IJAMT 2014 Weighted Job Score Heuristic algorithm
		int maxPrioIdx=-1;
		double maxBatchSumPrio=Double.NEGATIVE_INFINITY;
		double minBatchSumSlack=Double.POSITIVE_INFINITY;
		int counter=-1;
		
		// pick the highest priority batch for process
...
//		}
		...
		// pick the minimum slack batch for process
		for(Batch b:templist) {
			counter++;
			if(b.slackStat.sum()<minBatchSumSlack) {
				minBatchSumSlack=b.slackStat.sum();
				maxPrioIdx=counter;
			}
		}
