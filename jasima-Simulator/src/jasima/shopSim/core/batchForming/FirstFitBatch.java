 ...
 *          "$Id$"
 */
public class FirstFitBatch extends BatchForming {
		...
		for (int i = 0; i < numJobs; i++) {
			...
			for (int n = i + 1; n < numJobs; n++) {
				...
				for(int k=0;k<famBatchList.size();k++) {
					...
					//add to the first fit batch
					if (!j2.isFuture() && famBatchList.get(k).fullnessStat.sum()+1.0/(double)j2.getCurrentOperation().maxBatchSize<=1.0) {
						famBatchList.get(k).addToBatch(j2);
						isFormed=true;
						break;
					}						
				}
