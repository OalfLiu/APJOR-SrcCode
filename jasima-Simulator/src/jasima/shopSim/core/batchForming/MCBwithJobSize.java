 ...
 *          "$Id$"
 */
public class MCBwithJobSize extends BatchForming {
	...
	public MCBwithJobSize(double maxWaitRelative) {
		super();

		setMaxWaitRelative(maxWaitRelative);
	}

	public MCBwithJobSize() {
		this(0.0d);
	}
	...
	public void formBatches() {
		...
		{
			System.out.print("MCB possibleBatches.size() != 1");
		}
	...
	private Batch efficientBatching() {
		...
		for (List<Job> js : getOwner().getJobsByFamily().values()) {
			...
			for (int i = 0; i < js.size(); i++) {
				if (js.get(i).getArriveTime() - js.get(i).getShop().simTime() <= maxWait)
					arriveInTimeJobs+=1.0/(double)js.get(i).getCurrentOperation().maxBatchSize;
			}
			
			double rbs = Math.min(1.0, arriveInTimeJobs);
			if (rbs == maxRbs) {
				tie = true;
			} else if (rbs > maxRbs) {
				maxRbs = rbs;
		...
		if (!tie && !hasIncompatible) {
			Job j = maxFam.get(0);
			double mbs = 1.0;
			double arriveInTimeJobs = 0;
			for (int i = 0; i < maxFam.size(); i++) {
				if (maxFam.get(i).getArriveTime()
						- maxFam.get(i).getShop().simTime() <= maxWait)
					arriveInTimeJobs+=1.0/(double)maxFam.get(i).getCurrentOperation().maxBatchSize;
			}
...
//			assert b.numJobsInBatch() == Math.round(maxRbs * mbs);
			return b;
		} else {
			return null;
		}
...
//		q.setSequencingRule(getOwner().getBatchSequencingRule());
	...
	private void formBatches(Map<String, List<Job>> jobsByFamily) {
		...
		for (List<Job> famJobs : jobsByFamily.values()) {
			...
			double totfamsize=0;
			for(Job job:famJobs) totfamsize+=1.0/(double)job.getCurrentOperation().maxBatchSize;
			
			if (totfamsize < maxRBS)
				continue;
			...
			if (WorkStation.BATCH_INCOMPATIBLE.equals(o.batchFamily)) {
				...
				int i = 0;
				while (i < famJobs.size() && b.fullnessStat.sum() < 1.0) {
					if (famJobs.get(i).getArriveTime() - famJobs.get(i).getShop().simTime() <= maxWait
							&& b.fullnessStat.sum()+1.0/(double)famJobs.get(i).getCurrentOperation().maxBatchSize<=1.0)
						b.addToBatch(famJobs.get(i));
					i++;
				}

				if ((maxRBS) <= b.fullnessStat.sum()
						&& 0 < b.fullnessStat.sum()) {
					if (maxRBS  < b.fullnessStat.sum()) {
						possibleBatches.clear();
						maxRBS = b.fullnessStat.sum();
					}
					possibleBatches.add(b);
				}
			}
