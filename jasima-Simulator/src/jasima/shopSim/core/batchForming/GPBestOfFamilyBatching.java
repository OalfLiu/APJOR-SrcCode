 ...
 *          "$Id$"
 */
public class GPBestOfFamilyBatching extends BatchForming {
	...
	public GPBestOfFamilyBatching(PR batchSeqDR) {
		super();  
		this.batchSeqDR=batchSeqDR;
		rootNode = ((testGPRule)batchSeqDR).rootNode;
	}
	
	public GPBestOfFamilyBatching() { 
	}
	...
	public void formBatches() {
		...
		for(Batch b:templist) {
			initBatchData(b);
//			if((double)b.numJobsInBatch()/(double)b.op.maxBatchSize>0.5 || b.slackStat.min()<=1)
			possibleBatches.add(b);
		}
		
		if(possibleBatches.size()==0) return;
		
//		possibleBatches.setSequencingRule(this.getOwner().queue.getSequencingRule());
		Batch bestbat= possibleBatches.peekLargest();
		possibleBatches.clear();
		possibleBatches.add(bestbat);
