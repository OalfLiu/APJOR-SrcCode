...
public class SimSH {
	...
	public static double testPrioQueueAssertion() throws Exception {
		...
		sh35.setInitialSeed(1000);
		sh35.runExperiment();
//		sh35.printResults();
		
		Map<String, Object> res = sh35.getResults();
		
		double expAborted=(Integer)sh35.getResults().get("expAborted");
//		double wip=((SummaryStat)sh35.getResults().get("numWIP")).sum();
		double obj =((SummaryStat)sh35.getResults().get("weightedTardiness")).mean();
...
//		double wip=((SummaryStat)mre.getResults().get("numWIP")).sum();
	...
	public static double main(String[] args)
	{
		...
		for(int i=0;i<prAssignArray.length;i++)
		{
			...
			if(i<numHT)
			{
				...
				if(prAssignArray[i]==9) {//if GP Batching
					testGPRule batchSeqRule=new testGPRule(null);
					batchSeqRule.isTestStringGPRule=true; 
					batchSeqRule.isBatchSeqRule=true;
					batchSeqRules[ht]=batchSeqRule;
				}
...
//			if(i<2*numHT && i%2 == 0)
//			{
//				batchSeqRules[w]=decryptSeqPR(prAssignArray[i]);
//			}
//			else if(i<2*numHT  && i%2 != 0)
//			{
//				//batchFormRules[w]=decryptBatchFormPR(prAssignArray[i]);
//				batchFormRules[w]=decryptBatchFormPR(prAssignArray[i]);
//				w++;
...
//			else
//			{
//				seqRules[w]=decryptSeqPR(prAssignArray[i]);
//				w++;
//			}	
